...
import { p as propsFactory, O as makeRoundedProps, N as makeElevationProps, m as makeComponentProps, o as genericComponent, an as Ripple, q as useRtl, ar as useElevation, A as useTextColor, s as useRender, a1 as convertToUnit, bc as VScaleTransition, au as useRounded, a8 as useBackgroundColor, bd as makeFocusProps, ax as makeVInputProps, z as useProxiedModel, aA as useFocus, aC as VInput, a_ as VLabel, u as useRouter$1, b as useNuxtApp, e as useCookie, b8 as VProgressCircular, f as VTextField, V as VBtn, g as useRoute$1, ba as VSpacer, bb as getDecimals, X as createRange, _ as _export_sfc, c as useRuntimeConfig, $ as clamp, Y as keyValues } from './server.mjs';
  ...
  async setup(__props, { emit: __emit }) {
    ...
    async function onSubmit() {
      if (await v$.value.$validate()) {
        ...
        const { data: data2, error: error2, status: status2 } = await fetchWithToken(createPessoa, {
          method: "POST",
        ...
        if (status2.value === "error" && error2.value.statusCode === 500) {
          ...
          const pessoaIdValue = data2.value.id;
          pessoaId.value = pessoaIdValue;
          const pessoa_token = useCookie("pessoa_token");
          pessoa_token.value = data2.value.token;
    ...
    async function onUpdate() {
      ...
      const { status: status2 } = await fetchWithToken(`${updatePessoa}/${pessoaId.value}`, {
        method: "PUT",
        body: payloadFormated
      });
    ...
    async function validarCpf(cpf2) {
      const cpfFormated = removeFormatting(cpf2);
      if (cpfFormated.length === 11 && !isValidatingCpf) {
        ...
        try {
          const { error: error2, status: status2 } = await useFetch(routeValidaCpf, {
            method: "POST",
            body: payloadFormated
          }, "$fcbWHpl5XR");
          if (status2.value === "error" && error2.value.statusCode === 500) {
          ...
          console.error("Erro ao validar CPF:", error2);
        } finally {
                      ...
                      default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                        if (_push4) {
                          _push4(ssrRenderComponent(VTextField, mergeProps({
                            ...
                            required: "",
                            onBlur: ($event) => {
                              unref(v$).doc_identificacao.$touch();
                              validarCpf(unref(state).doc_identificacao);
                            },
  ...
  setup(__props, { emit: __emit }) {
    ...
    async function onSubmit() {
      if (await v$.value.$validate()) {
        ...
        const { data: data2, error, status } = await fetchWithToken(createPessoa, {
          method: "POST",
          body: payloadFormated
        });
    ...
    async function onUpdate() {
      ...
      const { data: data2, error, status } = await fetchWithToken(
    ...
    async function validarCnpj(cnpj2) {
      const cnpjFormated = removeFormatting(cnpj2);
      console.log(cnpj2);
      if (!isValidatingcnpj) {
        ...
        try {
          const { data: data2, error, status } = await useFetch(routeValidaCnpj, {
            method: "POST",
            body: payloadFormated
          }, "$kpCGWBoNdd");
          if (status.value === "error" && error.value.statusCode === 500) {
            $toast.error(" o cnpj j\xE1 est\xE1 cadastrado.");
            return;
          }
          if (data2.value.cpfValidation) {
            $toast.error("J\xE1 existe uma pessoa cadastrada com o cnpj digitado.");
          }
        } catch (error) {
                    ...
                    default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                      if (_push4) {
                        _push4(ssrRenderComponent(VTextField, mergeProps({
                          ...
                          label: "CNPJ",
                          required: "",
                          onBlur: ($event) => validarCnpj(unref(state).doc_identificacao),
                          onInput: unref(v$).doc_identificacao.$touch
                        }, ssrGetDirectiveProps(_ctx, _directive_mask, "##.###.###/####-##")), null, _parent4, _scopeId3));
                      } else {
                        return [
  ...
  async setup(__props, { emit: __emit }) {
    ...
    async function onSubmit() {
      if (await v$.value.$validate()) {
        ...
        const { data: data2, error, status } = await fetchWithToken(createDoc, {
          method: "POST",
          body: payloadFormated
        });
        if (status.value === "error" && error.value.statusCode === 500) {
    ...
    async function openScanner() {
      try {
        const { data: data2 } = await useFetch(acionarScanner, { method: "GET" }, "$cLGZtMLuJD");
      } catch (error) {
...
const _sfc_main$9 = {
  ...
  async setup(__props) {
    ...
    const { data: data2 } = ([__temp, __restore] = withAsyncContext(() => fetchWithToken(papeisApresentante, {
      method: "POST",
      body: { rotina: "CADASTRO_PJ" }
    })), __temp = await __temp, __restore(), __temp);
    papeisItems.value = data2.value;
    ...
    const createRepresentante = async () => {
      ...
      try {
        const { data: data22, error, status } = await fetchWithToken(criarAtoPessoa, {
          method: "POST",
          body: {
            ato_id: null,
        ...
        if (status.value === "success") {
          representante.id = data22.value.id;
  ...
  async setup(__props, { emit: __emit }) {
    ...
    async function onSubmitAdressNational() {
      if (await v$.value.$validate()) {
        ...
        const { data: data2, error, status: status2 } = await fetchWithToken(criarEnderecos, {
          method: "POST",
          body: payloadFormated
        });
        ...
        if (newCep.length === 8) {
          try {
            ...
            if (data2.value.logradouro && data2.value.bairro && data2.value.localidade && data2.value.uf) {
              state.logradouro = data2.value.logradouro;
              state.bairro = data2.value.bairro;
              state.complemento = data2.value.complemento;
...
//# sourceMappingURL=RegistroPessoas-71VnRREM.mjs.map
