...
import { V as VCard, a as VCardTitle, b as VCardText, c as VCardActions } from './VCard-CUYv16rw.mjs';
import { u as useFetch } from './fetch-DOsfzFYE.mjs';
import * as UTIF from 'utif';
import { _ as _sfc_main$d } from './FichaCard-D-EEskri.mjs';
...
const _sfc_main$b = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const isEditMode = ref(false);
    const pessoaId = ref(id2 || useCookie("pessoa-id").value);
    const state = reactive({
      ...initialState
    });
    function removeFormatting(value) {
    ...
    async function onUpdate() {
      ...
      const { data, error, status } = await fetchWithToken(
        `${updatePessoa}/${id2 || pessoaId.value}`,
        {
          method: "PUT",
          body: payloadFormated
        }
...
const _sfc_main$a = {
  ...
  async setup(__props, { emit: __emit }) {
    ...
    const pessoa_id = id2 ? Number(id2) : Number(useCookie("pessoa-id").value);
    const state = reactive({
    ...
    async function onUpdate(id22) {
      ...
      const { status } = await fetchWithToken(`${updateDoc}/${id22}`, {
        method: "PUT",
        body: payloadFormated
      });
      if (status.value === "success") {
...
const _sfc_main$8 = {
  ...
  async setup(__props, { emit: __emit }) {
    ...
    async function onUpdate(id3) {
      ...
      const { status: status2 } = await fetchWithToken(`${updateEndereco}/${id3}`, {
        method: "PUT",
        body: payloadFormated
      });
      if (status2.value === "success") {
  ...
  setup(__props) {
    ...
    async function captureBiometria(finger) {
      ...
      if (status.value === "success") {
        ...
        const bodyDigital = {
          user_id: useCookie("user-data").value.usuario_id,
          pessoa_id: Number(id2) || pessoa_id,
          dedo: finger,
          hash
        };
...
const _sfc_main$5 = {
  ...
  setup(__props) {
    ...
    const processarImagem = async (id22) => {
      var _a;
      if (!id22) return;
      const { data: imagemBiometria } = await useFetch(buscarPessoa, {
        method: "POST",
        body: { tipo: "foto", id: id22 }
      }, "$N16ZtKMkSh");
...
//# sourceMappingURL=RegistroPessoas-zaXtNyRf.mjs.map
