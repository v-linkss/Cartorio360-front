...
const loadEscreventes = async () => {
  try {
    ...
    const { data, error, status } = await useFetch(routeListarFormasReceb,      {
        method: "POST",
        body: JSON.stringify(body),  // Convertendo o objeto para JSON string
        headers: {
          "Content-Type": "application/json",  // Definindo o tipo do conteúdo
    ...
    console.log("Resposta da API:", data.value); // Adicionado para imprimir a resposta
    if (status.value === 200) {
      escreventesItems.value = data.value; // Atribuir os dados recebidos ao array
    } else {
      console.error("Erro ao carregar as formas de recebimento:", error.value);
    }
  } catch (error) {
    console.error("Erro ao realizar a requisição:", error);
  }
