  ...
  <v-container>
    <v-row v-if="!isVisualizar">
      <v-col cols="5" class="mt-6">
        <v-autocomplete label="Outros Atos" :items="outrosItems" item-title="descricao" item-value="id"
          v-model="state.outros_tj_id"></v-autocomplete>
      </v-col>
      ...
      <div>
        <img class="btn-pointer ml-2 mt-7" src="../../../assets/novo.png" style="width: 40px; cursor: pointer"
    ...
    <v-row>
      <v-col>
        <v-data-table style="height: 465px" :headers="headers" :items="outrosItemsTable">
          <template v-slot:item.actions="{ item }">
            <div v-if="!isVisualizar" style="display: flex; cursor: pointer; justify-content: flex-end"
              @click="deleteObservacao(item)" title="Deletar Observação">
