<template>
  <v-container>
      <v-row>
          ...
          <div
              ...
              title="Escanear"
              :class="{'botao-verde': arquivoAnexado}"
          >
              <img
                  class="mt-3"
  ...
  const arquivoAnexado = ref(false)
  ...
  async function openScanner() {
    try {
      await useFetch(acionarScanner, { method: 'GET' });
    } catch (error) {
  ...
  async function enviarArquivo() {
    try {
      await useFetch("http://localhost:3500/uploadAnexo", {
        method: 'POST',
  ...
  const createAnexo = async () => {
    const { data, status } = await useFetch(criarAtoAnexo, {
      method: "POST",
      body: {
        ato_id: props.ato_id,
