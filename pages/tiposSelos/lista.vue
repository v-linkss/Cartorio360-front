        ...
        <v-row style="margin-top: -6px;">
          ...
          <div>
            <nuxt-link :to="`/tiposSelos/atualizar/${item.id}`">
              <img style="width: 30px; height: 30px;cursor: pointer;" src="../../assets/editar.png" alt="Atualizar" />
            </nuxt-link>
          </div>

          <div @click="HandleDeleteSelo(item)" title="Deletar">
            <img v-if="item.excluido" style="width: 30px; height: 30px;cursor: pointer; margin-left: 7px;"
              src="../../assets/excluido.png" alt="Visualizar" title="Reativar" />
            <img v-else src="../../assets/mudarStatus.png" alt="Excluir" class="trash-icon"
              style="width: 30px; height: 30px;cursor: pointer;margin-left: 7px;" title="Excluir" />
          </div>
        </v-row>
      </template>
    </v-data-table>
